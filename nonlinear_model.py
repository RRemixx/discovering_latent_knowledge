...
class ResidualMLPProbe(nn.Module):
    def __init__(self, d, hidden_dim=128, n_layers=3):
        super().__init__()
    ...
    def forward(self, x):
        for layer, norm in zip(self.layers, self.norms):
            x = norm(x + layer(x))  # residual block
        return torch.sigmoid(self.output(x))



class CCS(object):
    def __init__(self, x0, x1, nepochs=2000, ntries=10, lr=1e-3, batch_size=-1, 
                 verbose=False, device="cuda", linear=False, weight_decay=0.01, var_normalize=False):
        # data
        self.var_normalize = var_normalize
        self.x0 = self.normalize(x0)
        self.x1 = self.normalize(x1)
        self.d = self.x0.shape[-1]
        print("Data shape: {}".format(self.d))
    ...
    def initialize_probe(self):
        ...
        elif self.linear_type == "res":
            self.probe = ResidualMLPProbe(self.d)
        else:
